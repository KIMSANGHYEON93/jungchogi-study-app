...
export function validateGeneratedDoc(doc, { originals = [] } = {}) {
  ...
  // 문항이 하나도 없는 생성물을 통과시키면 검증기가 "OK" 라고 말하고, 검수자가
  // 빈 파일에 reviewed:true 를 올린다. 아무 일도 하지 않는 파일이 검수 완료로
  // 커밋되고, 생성이 실패했다는 사실은 아무도 모른다.
  if (doc.items.length === 0) {
    add('items', 'items 가 비었습니다. 생성에 실패했거나 결과를 수거하지 못한 파일입니다.');
  }
