...
/**
 * `quiz_results` 에 저장되는 값 (클라이언트와 합의된 고정 계약).
 ...
 *   - 시도(attempted): `quizResults` 에 기록이 있거나 오답노트에 있는 문항 (중복 제거)
 *   - 오답(wrong):
 *       · `quizResults` 값이 'correct'/'incorrect' 면 **그 값을 그대로** 쓴다 (채점 결과).
...
export function runGetWeakCategories(snapshot) {
  ...
  // 2) 퀴즈 기록 — 정오를 아는 문항은 여기서 확정한다 (추정을 덮어쓴다)
  for (const [id, value] of Object.entries(snapshot?.quizResults ?? {})) {
