...
/**
 * 헤딩 비교용 정규화 — 앞의 `#`, 앞뒤 공백, 대소문자 차이를 없앤다.
 * 모델이 검색 결과의 헤딩을 그대로 되돌려주지 않고 `### ` 를 붙이거나
 * 대소문자를 흘려 쓰는 일이 잦아서, 그 정도 차이로 조회가 실패하지 않게 한다.
...
/**
 * 교재 섹션 하나의 **본문 전체**를 돌려준다 (`findRelatedSections` 의 excerpt 는 잘려 있다).
 * 문항 파일 3종은 색인에 없으므로 여기로 다른 문제의 정답이 새지 않는다.
 * @param {string} file `STUDY_FILES` 의 파일명
 * @param {string} heading 섹션 헤딩 (`#`·대소문자·공백 차이는 무시)
 * @returns {{file: string, heading: string, body: string}|null}
 */
export function getSection(file, heading) {
  sectionIndex ??= buildSectionIndex();

  const wanted = normalizeHeading(heading);
  if (!wanted) return null;

  const found = sectionIndex.find(
    (section) => section.file === file && normalizeHeading(section.heading) === wanted
  );
  return found ? { file: found.file, heading: found.heading, body: found.body } : null;
}

/**
 * @typedef {object} ProblemMeta
 * @property {string} source
 * @property {string} id
 * @property {string} title  지문(단답형·보강) 또는 제목(드릴)
 * @property {string} category 드릴은 언어를 카테고리로 쓴다
 * @property {string} lang   드릴의 언어, 나머지는 빈 문자열
 */

/**
 * 문항 **메타만** 돌려준다 — 정답·기대출력·함정·문제 코드는 담지 않는다.
 *
 * 플래너는 "무엇을 얼마나 공부할지"만 정하면 되고 정답을 알 필요가 없다.
 * 화이트리스트 방식(담을 필드를 열거)으로 만들어, 파서에 필드가 늘어도
 * 자동으로 새 나가지 않는다.
 *
 * @param {string} source
 * @param {{category?: string|null, ids?: string[]|null}} [filters]
 * @returns {ProblemMeta[]} `ids` 를 준 경우 **요청한 순서**를 유지한다
 */
export function listProblemMeta(source, filters) {
  ...
  const toMeta = (item) => ({
    ...
    // 드릴에는 category 가 없다. 교재도 언어(Part 1. C언어 …)로 나뉘어 있어 lang 이 그 역할을 한다.
    category: item.category ?? item.lang ?? '',
    lang: item.lang ?? '',
  });

  const ids = Array.isArray(filters?.ids) ? filters.ids : null;
  const selected = ids
    ? ids.map((id) => items.find((item) => item.id === id)).filter(Boolean)
    : items;

  const category = typeof filters?.category === 'string' ? filters.category.trim() : '';
  const metas = selected.map(toMeta);
  return category
    ? metas.filter((meta) => meta.category.toLowerCase() === category.toLowerCase())
    : metas;
}
