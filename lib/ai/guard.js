...
// 상한은 브라우저와 공유하는 `lib/ai/limits.js` 가 단일 출처다.
// 여기서 다시 export 하는 것은 기존 import 경로를 깨뜨리지 않기 위해서다.
export { MAX_USER_ANSWER_LENGTH } from './limits.js';
import { MAX_USER_ANSWER_LENGTH } from './limits.js';
...
export function validatePlanBody(body) {
  ...
  // 모의고사 채점 결과. 값 계약이 `quizResults` 와 같으므로 검증도 같다 —
  // 한쪽만 느슨하면 그쪽이 주입 통로가 된다.
  //
  // **없어도 되는 필드다.** `validateMap` 은 undefined·null 을 빈 맵으로 돌려주므로
  // 이 필드를 모르는 갱신 전 브라우저의 스냅샷도 400 없이 그대로 통과한다.
  // 필드를 늘렸다고 옛 클라이언트의 계획 생성이 막히면 안 된다.
  const examResults = validateMap(
    raw.examResults,
    'examResults',
    (v) => typeof v === 'string',
    sanitizeText
  );
  if (!examResults.ok) return bad(examResults.message);
