...
/** C0·C1 제어문자. 줄바꿈·탭은 남긴다 (본문 서식), 나머지는 지운다. */
...
export function validateGradeBody(body) {
  ...
  return { ok: true, value: { kind, source, id, userAnswer } };
}
...
function sanitizeText(value) {
  ...
  return value.replace(CONTROL_CHARS, '').slice(0, MAX_SNAPSHOT_TEXT);
}
...
function validateMap(raw, field, isValidValue, normalize) {
  ...
  for (const [key, item] of entries) {
    ...
    value[sanitizeText(key)] = normalize(item);
  }
